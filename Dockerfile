...
RUN make -j "$(($(nproc) + 1))"
WORKDIR /bitcoin/src
RUN strip bitcoin-util && strip bitcoind && strip bitcoin-cli && strip bitcoin-tx && strip bitcoin-wallet
...
COPY --from=builder /bitcoin/src/bitcoin-wallet /usr/local/bin
